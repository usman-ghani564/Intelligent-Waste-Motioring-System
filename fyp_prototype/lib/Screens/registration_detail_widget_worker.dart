...
class RegistationDetailsWidget extends StatefulWidget {
  ...
  dynamic complain2;
  String key2 = '';
  RegistationDetailsWidget(
      File i, Function sout, Function getuid, dynamic complain, String key) {
    _image = i;
    signout = sout;
    getuserid = getuid;
    getuserid = getuid;
...
class _RegistationDetailsWidgetState extends State<RegistationDetailsWidget> {
  ...
  void initState() {
    super.initState();
    getLoc();
    print('register complain widget' + widget.getuserid().toString());
  }
              ...
              TextButton(
                onPressed: () {
                  Navigator.pushReplacement(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          WorkerDashboard(widget.signout, widget.getuserid),
                    ),
                  );
                },
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  ...
                  widget.getuserid().then((uid) {
                    complaintProvider.getUserType(uid).then((value) {
                      print('usertype in widget :' + value);
                      if (value == 'worker') {
                        complaintProvider
                            .editComplaint(
                                widget.key2, widget.complain2, 'completed')
                            .then((value) {
